...
use Incutio\XMLRPC\ClientException;
...
class SSL extends Base
{
    ...
    public function setCertificate($certificateFile, $keyFile, $keyPhrase='')
    {
        ...
        $this->passphrase=(string)$keyPhrase;
    }
    ...
    public function query()
    {
        ...
        if (!($this->certFile === false)) {
            ...
            if ($this->debug) {
                echo "SSL Cert at : " . $this->certFile . "\n";
                echo "SSL Key at : " . $this->keyFile . "\n";
            }

            // See if we need to give a passphrase
            if (!($this->passphrase === '')) {
                curl_setopt($curl, CURLOPT_SSLCERTPASSWD, $this->passphrase);
            }
